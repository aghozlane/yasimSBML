...
typedef struct Especes {
  ...
  const char *compartment_id;  /*!< Compartment id */
...
void Especes_save(pEspeces, int, double, const char *, const char *);

/* Affichage des infos et ratio des especes */
void Especes_print(pEspeces, int);

/* Affichage des infos des especes */
void Especes_print_2(pEspeces, int);

/* Trouve une espece a partir de son ID */
int Especes_find(pEspeces, const char *, /*const char *,*/ int);
...
int Especes_getNbreactions(pEspeces, int);
