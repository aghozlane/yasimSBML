#!/bin/bash
###############################################################################################
# This file is part of yasimSBML (http://www.labri.fr/perso/ghozlane/metaboflux/yasimSBML.php)#
...
help()
{
  ...
  echo
  echo '             -I : Compile the software in yasimSBML-<version>/bin/'
...
if [ ! -f src/especes.c -o ! -f include/especes.h ]
then
...
then
  ...
  if [ $? -ne 0 ]
  then
    echo To be able to compile/install yasimSBML, you must install cmake and/or update the PATH variable >&2
    exit 1
  fi
